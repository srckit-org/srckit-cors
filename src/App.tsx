...
import CORSPreflight from './components/CORSPreflight';
import CORSExplain from './components/CORSExplain';

const tabs: { label: string; icon: ReactElement; component: ReactNode }[] = [
  { label: 'Tester', icon: <GppGoodIcon />, component: <CORSTester /> },
  { label: 'Preflight', icon: <FlightTakeoffIcon />, component: <CORSPreflight /> },
  { label: 'Explain', icon: <HelpIcon />, component: <CORSExplain /> },
];
...
<AppBar position="sticky" elevation={0} className="border-b border-gray-200" style={{ background: '#f8fafc', color: '#1e293b' }}>
        <Toolbar className="min-h-12 px-2 md:px-4">
          ...
          <Typography variant="h6" sx={{ fontWeight: 800, letterSpacing: "-0.5px", color: "#111827" }} className="text-lg md:text-xl" style={{ color: '#1e293b' }}>CORS Tester</Typography>
